...
BEGIN
  ...
  DECLARE v_finished INTEGER DEFAULT 0;
	...
	DECLARE error_log_cursor CURSOR FOR SELECT A.transfer_id, B.transfer_id, A.pay_amount, B.transfer_amount, A.pay_time, B.transfer_time, A.client_id, B.client_id 
		FROM pay_log A LEFT JOIN transfer_log B ON A.transfer_id = B.transfer_id 
		WHERE (A.bank_id = b_id OR B.bank_id = b_id) AND (TO_DAYS(A.pay_time) = TO_DAYS(date) OR TO_DAYS(B.transfer_time) = TO_DAYS(date))
		UNION
    ...
    find_error: LOOP
		FETCH error_log_cursor INTO temp_A_transfer_id, temp_A_transfer_id, temp_A_pay_amount, temp_B_transfer_amount, temp_A_pay_time, temp_B_transfer_time, temp_A_client_id, temp_B_client_id;
		SELECT v_finished, temp_A_transfer_id, temp_A_transfer_id, temp_A_pay_amount, temp_B_transfer_amount, temp_A_pay_time, temp_B_transfer_time, temp_A_client_id, temp_B_client_id;
				IF v_finished = 1 THEN 
						LEAVE find_error;
				END IF;
				
				IF temp_A_transfer_id = null THEN
				...
				END IF;
				IF temp_B_transfer_id = null THEN
				...
				END IF;
				IF temp_A_pay_amount != temp_B_pay_amount THEN 
						INSERT error_log(account_time, bank_id, transfer_id, client_id, bank_amount, enterprise_amount, account_info)
