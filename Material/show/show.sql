...
-- 2.1新电表抄表一次，时间为9月
-- 2.1.1默认抄用户1表1
CALL read_meter(str_to_date('01-09-2017', '%d-%m-%Y'), 2008, 100, 1); -- 默认抄用户1表1(用户1表1:2008, 用户1表2:2009, 用户2表1:2010)
-- 2.1.2查看欠费清单
...
-- 2.2.1连续抄表
CALL read_meter(str_to_date('01-12-2017', '%d-%m-%Y'), 2008, 100, 1);
CALL read_meter(str_to_date('01-01-2018', '%d-%m-%Y'), 2008, 120, 1);
CALL read_meter(str_to_date('01-04-2018', '%d-%m-%Y'), 2008, 100, 1);
CALL read_meter(str_to_date('01-06-2018', '%d-%m-%Y'), 2008, 120, 1);
CALL read_meter(str_to_date('01-07-2018', '%d-%m-%Y'), 2008, 100, 1);
-- 2.2.2查询出所有欠费超过半年的用户
SELECT client_id, client_name, address, balance
FROM cost_log JOIN (device JOIN client USING(client_id)) USING(device_id)
WHERE (pay_state = '未缴' OR pay_state = '部分缴') AND TIMESTAMPDIFF(MONTH, payable_date, NOW()) >= 6;
...
-- 3.2缴费
-- 3.2.1缴费缴纳小于等于0的数
CALL pay(1005, -50, 'CMB', 2008);
-- 3.2.2缴费不存在电表
CALL pay(1005, 200, 'CMB', 20088);
-- 3.2.3查看当日应缴费用和应收费用
...
CALL check_total('CMB', 2, 500, str_to_date('20-02-2019', '%d-%m-%Y'), @state);
SELECT @state;
-- 4.1.1查看对总账存储过程
